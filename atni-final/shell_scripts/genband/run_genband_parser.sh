...
fileCount=`hadoop fs -count $IN_PATH | awk '{print $2}'`

if [ $fileCount -eq 0 ]; then
	echo 'No new Genband files to parse...'
	exit 0
fi

path="genband_main.py $IN_PATH"
...
cd $ATNI_REPO/common
sudo -u gtt python check_duplicate_file.py genband

cd $ATNI_REPO/genband/

sudo -u gtt spark-submit \
--master yarn  \
--py-files $ATNI_REPO/bin/genband.zip,$ATNI_REPO/bin/common.zip \
./genband_main.py $IN_PATH
